...
We're back to server side again, this one is fairly simple. Signed chat needs to account for servers wanting to format the message in their own ways, adding tags to the player for ranks, message colouring, etc. This system can completely rewrite the message, except if you modify the message itself, you get a little gray bar and if the user hovers, they get a little balloon telling them what the original message was. To avoid showing this on every message, Mojang checks if the displayed message contains the signed message, and if it does, it marks it as unmodified. The server uses this to transform the player's signed message from "Hello!" to "\<Player123\> Hello!" without tripping the modified indicator. There is no modified indicator, no hovering over the message to see what was signed, and it shows up in the report screen as the server sent it.

This can be exploited to gaslight a player about what you actually said. For example, if you sign "hi!", the server can extend that to "hi! I griefed your base last night". This means the player sees the extended message, cannot tell it's been modified, and will be replying as if you actually sent the provocative message. If they decide to report you, the extended message will appear in the report window, and at no point does the user see the signed content. However, Mojang will receive only the signed content and have no idea about the extended message. This obviously also works in the inverse, you can extend a message with the goal of provoking a player into saying something reportable.
